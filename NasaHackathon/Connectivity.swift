...
class ConnectivityService : NSObject {
    ...
    private let DataServiceType = "hash"

    private let myPeerId = MCPeerID(displayName: UIDevice.current.name)
    private let serviceAdvertiser : MCNearbyServiceAdvertiser
    private let serviceBrowser : MCNearbyServiceBrowser

    var delegate : HashServiceDelegate?
    ...
    func send(hashData : String) {
        ...
        if session.connectedPeers.count > 0 {
            do {
                try self.session.send(hashData.data(using: .utf8)!, toPeers: session.connectedPeers, with: .reliable)
            }
            catch let error {
...
extension ConnectivityService : MCSessionDelegate {
    ...
    func session(_ session: MCSession, didReceive data: Data, fromPeer peerID: MCPeerID) {
        ...
        self.delegate?.hashChanged(manager: self, hashData: str)
    }
